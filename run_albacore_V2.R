...
devtools::load_all('~/Dropbox/APL/GitHub/R/HMMoce')
#devtools::install_github('camrinbraun/HMMoce', ref='dev')
library(HMMoce)
library(raster)
library(tidyverse)
library(fasttime)
devtools::load_all('~/Dropbox/APL/Hawaii/R/Collocation/tags2etuff') ## for etuff functionality
devtools::load_all('~/Dropbox/APL/Hawaii/R/Collocation/analyzePSAT')
#source('../analyzePSAT/R/make360.R')
library(fields) ## for quick mapping

meta <- read.table('~/Dropbox/APL/GitHub/R/nip_drake/RawData/all_tag_meta.csv', sep=',', header=T, stringsAsFactors = F)
...
which(meta$manufacturer=="Wildlife Computers")
which(meta$manufacturer=="Lotek")
# i sequence
# 1-6 Lotek, 7-8 WC, 9 Lotek, 10-11 WC, 12-22 Lotek, 23-24 WC

### Done through i == 24

# Manually assign i value and run through
i <- 24
  
  if (meta$manufacturer[i] != 'Lotek'){
    drop_light[[i]] <- NA
    print("Stop here for this entry")
  }
    ...
    by_hand <- identify(tr$lon2, tr$DateTime,col=2)
    
    if (length(auto_idx) > 0){
    drop_light[[i]] <- list(drop_auto = drop_auto, by_hand = by_hand)
    } else {
    drop_light[[i]] <- by_hand
    }
    quartz.save(paste0("drop_light_i", i, "_", meta$instrument_name[i], '.png'))

  # When finished with all entries, save the indexing file
  saveRDS(drop_light,"~/Dropbox/APL/Albacore/R/drop_light.rds")
