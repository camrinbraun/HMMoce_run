...
# SETWD
setwd('~/Documents/WHOI/RCode/HMMoce_run/data/121325/') 
#setwd('C:/RData/HMMoce_run/data/121325/')
load('121325_hmm_geo.RData')
...
sst.dir <- '~/Documents/WHOI/RCode/HMMoce_run/env_data/121325/sst/'
get.env(sst.udates, type = 'sst', spatLim = sp.lim, save.dir = sst.dir)

# HYCOM DATA
hycom.dir <- paste('C:/RData/HMMoce_run/env_data/121325/hycom/')
...
t6<-Sys.time()
# try i = 51
L.qwoa <- calc.woa(pdt.try, ptt, woa.data = woa.quarter, focalDim = 9, dateVec = dateVec, use.se = F)
t7<-Sys.time()
...
#t3<-Sys.time()
setwd(myDir)
base::save.image('121325_hmm_geo.RData')
...
L.res2 <- resample.grid(L.rasters2, L.rasters2$L.sst)
#L.res3 <- resample.grid(L.rasters3, L.rasters3$L.sst)
...
par0 <- calc.param(migr.spd = 6, resid.spd = 1, g = g.mle)
...
#----------------------------------------------------------------------------------#
par0 <- calc.param(migr.spd = 6, resid.spd = 1, g = g)
D1 <- unlist(par0[1:2]) # parameters for kernel 1. this is migratory behavior mode
D2 <- unlist(par0[3:4]) # parameters for kernel 2. resident behavior mode
K1 <- gausskern(D1[1], D1[2], muadv = 0)
K2 <- gausskern(D2[1], D2[2], muadv = 0)
...
## END
#=======================================================================================#
spot <- read.table('121325_crawl_track.csv',sep=',',header=T)

pdf('check_lydia_L.pdf',width=12,height=10)
par(mfrow=c(2,2))
for (i in 1:length(dateVec)){
  ...
  image.plot(lon, lat, L[i,,]); world(add=T); points(spot$lon[i], spot$lat[i])
}
dev.off()
