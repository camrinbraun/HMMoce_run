...
# SETWD
setwd('~/Documents/WHOI/RCode/HMMoce_run/data/141257/') 
#setwd('C:/RData/HMMoce_run/data/141257/')
load('141257_hmm_geo.RData')
...
iniloc <- data.frame(matrix(c(15, 10, 2015, 41.637, -69.706, 
                              13, 4, 2016, 37.751, -71.649), nrow = 2, ncol = 5, byrow = T))
...
sst.dir <- 'C:/RData/HMMoce_run/env_data/141257/sst/'
get.env(sst.udates, type = 'sst', spatLim = sp.lim, save.dir = sst.dir)

# HYCOM DATA
hycom.dir <- paste('C:/RData/HMMoce_run/env_data/141257/hycom/')
...
#t3<-Sys.time()
setwd(myDir)
base::save.image('141257_hmm_geo.RData')
...
mako.light <- sample(light.udates, round(.25 * length(light.udates), 0))
mako.sst <- sample(sst.udates, round(.5 * length(sst.udates), 0))
